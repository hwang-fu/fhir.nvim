...
describe("index.flavor", function()
  it("classifies reference flavors", function()
    assert.are.equal("contained", index.flavor("#p1"))
    assert.are.equal("urn-uuid", index.flavor("urn:uuid:9a2b"))
    assert.are.equal("absolute", index.flavor("https://example.org/fhir/Patient/1"))
    assert.are.equal("relative", index.flavor("Patient/1"))
    assert.are.equal("unknown", index.flavor("???"))
  end)
end)
