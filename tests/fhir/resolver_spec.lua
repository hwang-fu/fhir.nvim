...
describe("LocalBufferResolver.resolve_resource", function()
  it("returns the target resource for urn/relative/absolute", function()
    local idx = index.get(h.fixture_buf("bundle_urn.json"))
    local occ = ref_named(idx, "urn-uuid")
    assert.are.equal(idx.by_identity["urn:uuid:p1"], resolver.resolve_resource(occ, idx))
  end)

  it("returns nil for contained and for unresolvable references", function()
    local cidx = index.get(h.fixture_buf("contained.json"))
    assert.is_nil(resolver.resolve_resource(cidx.references[1], cidx))
    local uidx = index.get(h.fixture_buf("unresolvable.json"))
    assert.is_nil(resolver.resolve_resource(uidx.references[1], uidx))
  end)
end)
